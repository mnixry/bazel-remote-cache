...
async function main(): Promise<void> {
  ...
  const level = process.env.BAZEL_REMOTE_CACHE_LOG_LEVEL?.trim() || 'info'
  const namespace =
  ...
  const app = buildBazelRemoteCacheServer({ backend, logger: log })

  log.info({ host, port, namespace, storeDir }, 'starting server')
